...
@app.post("/projects/{project_id}/teams/{team_id}/teammermbers", status_code = status.HTTP_201_CREATED)
def get_member_team(project_id: int, team_id: int, user_id : int = Depends(get_current_user)):
    # check if project exits
    res = runSQL("""SELECT * FROM projects WHERE project_id = %s""",(project_id,))
    if not res:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"the project with id {project_id} can t be found")
...
@app.post("/projects/{project_id}/teams/{team_id}/addmember", status_code = status.HTTP_201_CREATED)
def add_member_team(project_id: int, team_id: int, member_id: int, user_id : int = Depends(get_current_user)):
    # check if project exits
    res = runSQL("""SELECT * FROM projects WHERE project_id = %s""",(project_id,))
    if not res:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"the project with id {project_id} can t be found")
...
@app.delete("/projects/{project_id}/teams/{team_id}/removemember", status_code = status.HTTP_201_CREATED)
