...
def add_post_comment(post_id: int, comment: Comment, user_id : int = Depends(get_current_user)):
    ...
    runSQL("""INSERT INTO users_comments_posts (comment_owner_id, post_id, comment_body, comment_date) VALUES (%s,%s,%s,NOW());""" ,(user_id, post_id, comment.comment_body))
...
def edit_post_comment(comment_id: int, comment: Comment, user_id : int = Depends(get_current_user)):
    ...
    res = runSQL("""UPDATE users_comments_posts SET comment_body = %s WHERE comment_id = %s """,(comment.comment_body, comment_id))
...
def delete_post_comment(post_id: int, comment_id: int, user_id : int = Depends(get_current_user)):
    ...
    res = runSQL("""DELETE FROM users_comments_posts WHERE post_id = %s AND comment_id = %s""",(post_id, comment_id))
