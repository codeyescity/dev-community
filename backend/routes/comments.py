...
def delete_post_comment(post_id: int, comment_id: int, user_id : int = Depends(get_current_user)):
    res = db.runSQL("""SELECT * FROM users_comments_posts WHERE post_id = %s AND comment_id = %s""",(post_id, comment_id))
    if not res:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"the comment with id {comment_id} can t be found")
    if res[0]["comment_owner_id"] != user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to perform requested action")
...
def like_comment(post_id: int , comment_id: int, user_id : int = Depends(get_current_user)):
    ...
    res = db.runSQL("""SELECT * FROM users_likes_comments WHERE comment_id = %s AND comment_liker_id = %s""",(comment_id, user_id))
    if(res):
        # remove like
        db.runSQL("""DELETE FROM users_likes_comments WHERE comment_id = %s AND comment_liker_id = %s""",(comment_id, user_id))
        db.runSQL("""UPDATE users_comments_posts SET comment_number_likes = comment_number_likes - 1 WHERE comment_id = %s""",(comment_id,))
        return Response(status_code = status.HTTP_204_NO_CONTENT)
    else:
        # add like
        db.runSQL("""INSERT INTO  users_likes_comments (comment_id,comment_liker_id) VALUES (%s,%s)""",(comment_id, user_id))
        db.runSQL("""UPDATE users_comments_posts SET comment_number_likes = comment_number_likes + 1 WHERE comment_id = %s""",(comment_id,))
